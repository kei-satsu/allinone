...
const handleExportImage = async () => {
  ...
  try {
    ...
    // ညာဘက်နှင့် အောက်ဘက်တွင် ပိုချင်သည့် Space (Pixel) ပမာဏများ
    const paddingRight = 40;  // ညာဘက်အကွာအဝေး
    const paddingBottom = 40; // အောက်ဘက်အကွာအဝေး

    // Original Scroll Width/Height ထဲသို့ Padding ပမာဏ ပေါင်းထည့်ခြင်း
    const exportWidth = node.scrollWidth + paddingRight;
    const exportHeight = node.scrollHeight + paddingBottom;
    ...
    link.download = `Orders_Report_${new Date().toISOString().split('T')[0]}.png`;
    link.href = dataUrl;
    link.click();
  } catch (err) {
    console.error('Failed to export image:', err);
    alert('Image Export ထုတ်ရာတွင် အမှားအယွင်းရှိနေပါသည်။');
  }
};
  ...
  // ရွေးချယ်ထားသော Sender အမည်
const [selectedSender, setSelectedSender] = useState<string | null>(null);

// ရွေးထားသော ရက်စွဲတွင် ရှိနေသည့် Sender များ စာရင်း ( Count ပါ ထည့်ထားနိုင်ပါသည် )
const [senderList, setSenderList] = useState<{ name: string; count: number }[]>([]);
...
const fetchSenders = async (branchCode: string) => {
  const { data, error } = await apiClient
    .from('orders')
    .select('sender_name')
    .eq('is_deleted', false)
    .eq('branch', branchCode)
    .not('cleared_date', 'is', null) // cleared_date ရှိရမည်
    .neq('status', 'Delivered')     // Status Delivered မဖြစ်ရ
    .neq('status', 'Settled')
    .neq('status', 'Returned');       // Status Settled မဖြစ်ရ
  ...
  const formattedSenders = Object.keys(counts).map((name) => ({
    name,
    count: counts[name],
  }));

  setSenderList(formattedSenders);
  
  // အစပိုင်းတွင် ပထမဆုံး Sender ကို Auto Select လုပ်ပေးခြင်း (သို့မဟုတ် null ထားခြင်း)
  if (formattedSenders.length > 0) {
    setSelectedSender(formattedSenders[0].name);
  } else {
...
useEffect(() => {
  ...
  const fetchData = async ({
    ...
    let query = apiClient
      ...
      .not('cleared_date', 'is', null) // cleared_date ရှိသော Order ဖြစ်ရမည်
      .neq('status', 'Delivered')     // Delivered မဟုတ်ရ
      .neq('status', 'Settled')         // Settled မဟုတ်ရ
      .neq('status', 'Returned')       // Returned မဟုတ်ရ
    ...
    if (selectedSender) {
  query = query.eq('sender_name', selectedSender);
}
...
useEffect(() => {
  ...
  // 🟢 ၁။ Date သို့မဟုတ် Branch ပြောင်းပါက Sender စာရင်း အသစ်ပြန်ယူခြင်း
  // Sender List ခေါ်ယူခြင်း
useEffect(() => {
  if (userBranch) {
    fetchSenders(userBranch);
  }
}, [userBranch]);

// Table Data ခေါ်ယူခြင်း
useEffect(() => {
  if (!userBranch || !selectedSender) return;
...
}, [selectedSender, globalSearch, userBranch, colFiltersKey]);

  // အောက်မှာ မူလပါပြီးသား Context Menu handler ဆက်ရှိနေပါမည်
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(timer);
  }, [globalSearch, userBranch, colFiltersKey]);
  ...
  <span className="uppercase">{userBranch === 'MDY' ? 'Mandalay' : userBranch === 'YGN' ? 'Yangon' : 'Main'} Office | Prepaid စရင်း</span>
...
<button 
  ...
  className="bg-blue-600 hover:bg-blue-700 text-white font-medium px-3 py-1.5 rounded-md transition-all text-xs flex items-center gap-1.5 shadow-sm"
>
  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
  </svg>
...
<div className="flex min-h-0 flex-1 flex-col overflow-hidden md:flex-row">
  ...
  <div className="w-full md:w-64 bg-white border-r border-gray-200 p-3 overflow-y-auto shrink-0 max-h-48 md:max-h-full">
    <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
      Senders ({senderList.length})
    </h3>
    
    <div className="space-y-1">
      {senderList.length === 0 ? (
        <p className="text-xs text-gray-400 italic py-2">ဒီရက်မှာ အော်ဒါမရှိပါ</p>
      ) : (
