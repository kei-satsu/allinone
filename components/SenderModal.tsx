...
import { createSender, updateSender } from "@/lib/sendersApi";
...
export default function SenderModal({
  ...
  const handleSave = async () => {
    ...
    if (mode === "add") {
      ...
      try {
        resultData = (
          await updateSender(editData.id, {
            name: name.trim(),
            phone: phone.trim(),
            Address: address.trim(),
            "kpay-no": kpayNo.trim(),
        ...
        <div className="flex items-center justify-between gap-4 mb-5">
          <div>
            ...
            <p className="text-sm text-gray-500">
              {mode === "edit"
                ? "ပို့ဆောင်သူ၏ အချက်အလက်များကို ပြင်ဆင်ပါ။"
                : "ပို့ဆောင်သူအသစ် စာရင်းသွင်းပြီး စနစ်ထဲတွင် ချက်ချင်း အသုံးပြုပါ။"}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-2xl border border-blue-300 bg-blue-50/40 px-4 py-3 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
              placeholder="KPay ဖုန်းနံပါတ်"
            />
          </div>
